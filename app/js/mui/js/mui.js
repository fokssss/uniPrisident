...
(function($, window, document, name, undefined) {
   ...
   var handle = function(event, target) {
    ...
    window.addEventListener('tap', function(e) {
        ...
        // 自定义部分
        var func = targetTab.dataset.func || '';
        if(func !== ''){
        	eval(func);
        }
        // 自定义结束
...
(function($, window, document) {
    ...
    var createInput = function(placeholder,title) {
    	var str = '';
    	if(title == '加签' || title == '呈报' || title == '改派'){
    	   str = '<div id="nickname" onclick="showJq()"><span class="selected">选择'+title+'人</span> <span class="mui-pull-right mui-icon mui-icon-arrowdown"></span></div>';
    	}

        return str + '<div class="' + CLASS_POPUP_INPUT + '"><textarea autofocus placeholder="' + (placeholder || '') + '"></textarea></div>';
    };
    ...
    var createPrompt = function(message, placeholder, title, btnArray, callback, type) {
        ...
        if (!$.os.plus || type === 'div') {
            return createPopup(createInner(message, title || '提示', createInput(placeholder,title)) + createButtons(btnArray || ['取消', '确认']), callback);
        }
